...
def submit_application():
    ...
    try:
        ...
        return jsonify({"error": str(e)}), 500app.route('/api/admin/login', methods=['POST'])
def admin_login():
    """Login do admin"""
    try:
        data = request.get_json()
...
def get_stats():
    ...
    return jsonify({
        ...
        "applications_today": len([app for app in APPLICATIONS if "2024-08-19" in app.get('created_at', '')]),
    ...
    if path != "" and os.path.exists(os.path.join('src/static', path)):
        ...
        try:
            ...
            <body>
                ...
                <p class="info">Backend completo funcionando com dados em memória.</p>
                <p class="info">Formulários e login admin operacionais.</p>
...
if __name__ == '__main__':
    ...
    print(f"🚀 Iniciando Airbus Careers na porta {port}")
    print(f"📊 {len(MOCK_JOBS)} vagas carregadas")
    print(f"🔐 Admin: {ADMIN_CREDENTIALS['username']} / {ADMIN_CREDENTIALS['password']}")
